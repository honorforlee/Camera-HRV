...
export default class HomePage extends Component {
  ...
  constructor(props) {
    ...
    const listener = myModuleEvt.addListener('sayHello', (data) => this.update(data));
  }
    ...
    update(data){
        ...
        // New RMS alg:
        var rms = Math.pow(signal[WINDOW_SIZE-1][1]-signal[WINDOW_SIZE-2][1],2) + Math.pow(signal[WINDOW_SIZE-2][1]-signal[WINDOW_SIZE-3][1],2) + Math.pow(signal[WINDOW_SIZE-3][1]-signal[WINDOW_SIZE-4][1],2);
        rms = Math.sqrt(rms);
        rms /= 3; 
        new_val = rms;
        //--------
        chart.push({time: cnt, value: new_val}); 
        prev = int_data;
    ...
    calculateHRV(){
        ...
        for(i=1 ; i<rr.length-1 ; i++){
            if(Math.abs(rr[i].time - rr[i-1].time) < 0.5*avg_rr || Math.abs(rr[i+1].time - rr[i].time) < 0.5*avg_rr || Math.abs(rr[i].time - rr[i-1].time) > 1.5*avg_rr || Math.abs(rr[i+1].time - rr[i].time) > 1.5*avg_rr)
                continue;
            else{
                            ...
                            <View style={{flex: 4}}>
                                <VictoryChart theme={VictoryTheme.material}>
                                    ...
                                    <VictoryLine
                                        ...
                                        domain={{y: [this.state.int_avg-80, this.state.int_avg+80]}}
                                        data={chart}
                                        x="time"
                                        y="value" />
